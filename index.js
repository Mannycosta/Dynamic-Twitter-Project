...
var selectedUser = users[getParameter('user')] || user1;
...
var tweetCount = selectedUser.tweets.length;
...
photoContainer.style.backgroundImage = `url('${selectedUser.coverPhotoURL}')`;
var profileImage = document.querySelector('.profile-image');
profileImage.style.backgroundImage = `url('${selectedUser.avatarURL}')`;
...
profileInfo.innerHTML = `
  ...
  <div class="joined-date">
    <p><i class="bi bi-calendar3"></i> Joined ${selectedUser.joinedDate}</p>
  </div>
  <div class="following">
    <p><span class="follow-num">${abbreviateNumber(
...
var tweetText = [];
for (let tweet of selectedUser.tweets) {
...
if (selectedUser.tweets.length > 0) {
	selectedUser.tweets.forEach(function (tweet, i) {
		var rowDiv = document.createElement('div');
		rowDiv.className = 'tweet-content';
		rowDiv.innerHTML = `
    <div class="tweet-photo">
      <img class="photo" src='${selectedUser.avatarURL}'>
    </div>
    <div class="tweet-stats">
      <div class="tweet-upper">
        <div>
          <h3 class="tweet-display-name">${getDisplayName(selectedUser)}</h3>
        </div>
        <div>
          <p class="tweet-user-name">${selectedUser.userName}</p>
        </div>
        <div>
	...
	tweets.innerHTML = `
    <p class="no-tweets">${selectedUser.userName} hasnt tweeted yet</p>`;
}
